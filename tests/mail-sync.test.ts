...
test('resolveProviderForMailbox 优先使用 mailbox 上绑定的 provider_id', () => {
  const provider = resolveProviderForMailbox(
    { email: 'bound@hidden-provider.example', provider_id: 'p-2' },
    [
      { id: 'p-1', name: 'A', domain: 'https://a.example.com/api/public', token: 't1', email_domain: 'hidden-provider.example' },
...
test('resolveProviderForMailbox 在未绑定 provider 时按 email_domain 推断', () => {
  const provider = resolveProviderForMailbox(
    { email: 'guess@hidden-provider.example', provider_id: null },
    [
      { id: 'p-1', name: 'A', domain: 'https://a.example.com/api/public', token: 't1', email_domain: 'hidden-provider.example' },
...
test('resolveProviderForMailbox 兼容旧 provider 数据，从 domain 自动推断邮箱域名', () => {
  const provider = resolveProviderForMailbox(
    { email: 'legacy@hidden-provider.example', provider_id: null },
    [
      { id: 'p-1', name: 'Primary Provider', domain: 'https://mail.hidden-provider.example', token: 't1', email_domain: null },
      { id: 'p-2', name: 'Backup Provider', domain: 'https://mail.backup-provider.example', token: 't2', email_domain: null },
    ]
  );
...
test('syncMailboxFromProvider 调用 emailList 并只写入新邮件', async () => {
  ...
  const result = await syncMailboxFromProvider({
    mailboxEmail: 'target@hidden-provider.example',
    provider: {
      id: 'p-1',
      name: 'Primary Provider',
      domain: 'https://mail.hidden-provider.example/api/public',
      token: 'secret-token',
      email_domain: 'hidden-provider.example',
    },
        ...
        data: [
          {
            ...
            sendName: 'Alpha',
            subject: 'old message',
            toEmail: 'target@hidden-provider.example',
  ...
  assert.equal(requests[0].url, 'https://mail.hidden-provider.example/api/public/emailList');
  ...
  assert.deepEqual(saved[0], {
    mailbox_email: 'target@hidden-provider.example',
    message_id: 'upstream:1002',
    from_addr: 'beta@example.com',
    from_name: 'Beta',
    to_addr: 'target@hidden-provider.example',
    subject: 'new message',
    text_body: 'new',
...
test('syncMailboxFromProvider 兼容旧 provider domain，自动补 api/public 前缀', async () => {
  ...
  await syncMailboxFromProvider({
    ...
    provider: {
      id: 'p-1',
      name: 'Primary Provider',
      domain: 'https://mail.hidden-provider.example',
      token: 'secret-token',
      email_domain: null,
    },
  ...
  assert.equal(requestedUrl, 'https://mail.hidden-provider.example/api/public/emailList');
});
...
test('syncAllMailboxesFromProviders 只同步能匹配到 provider 的邮箱', async () => {
  ...
  const stats = await syncAllMailboxesFromProviders({
    mailboxes: [
      { email: 'first@hidden-provider.example', provider_id: null },
  ...
  assert.deepEqual(calls, [
    { mailboxEmail: 'first@hidden-provider.example', providerId: 'p-1' },
  ]);
